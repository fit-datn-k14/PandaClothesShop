        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    # get product variations by cart_id
                    product_variation = []
...
def activate(request, uidb64, token):
    # activate link at email address
    try:
...
def forgot_password(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            user = Account.objects.get(email__exact=email) # check email exact
            ...
            messages.success(request, 'Password reset just have sent to your email')
            return redirect('login')
        else:
...
def resetpassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.set_password(password) # resset password
            user.save()
